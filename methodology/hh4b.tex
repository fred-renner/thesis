...
\section{Dataset and Monte Carlo Simulation}\label{sec:mc_simulation}
...
\begin{table}[htbp]
    ...
    \label{tab:abcd}
\end{table}
This gives the four orthogonal regions shown in table \ref{tab:abcd}. Hence, the background in the \ac{sr} is estimated with a weight extracted from the \ac{cr} with statistical uncertainty
\begin{equation}
    N_\text{SR}^\text{2 GN2X}=\frac{N_\text{CR}^\text{2 GN2X}}{N_\text{CR}^\text{1 GN2X}} N_\text{SR}^\text{1 GN2X} = w_\text{CR} N_\text{SR}^\text{1 GN2X}= \qty[separate-uncertainty = true]{3.79(045)e-3}{} \times N_\text{SR}^\text{1 GN2X}.
\end{equation}
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{tomatos_cls_5_2500_slope_50_ratios_0p8_0p5_NOSYS.VR_xbb_2_compareABCD}
    \caption[]{Comparison of \ac{nn} response in the \ac{vr} for the 2 GN2X tagged Higgs candidates and the background estimate from multiplying the 1 GN2X tagged selection with $w_\text{CR}$. Except for the first bin ratio of 5.04 the uncertainties are within the statical uncertainty of the 2 GN2X selection.} 
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrr}
        ...
        Initial events          & 1147720630 & 100.00000              & 100.00000                \\\hline
        Trigger                 & 280935686  & 24.47771               & 24.47771                 \\
        Two fat jets            & 72439244  & 25.78499               & 6.31157                  \\
        Fat jet $\pt$           & 53743119   & 74.19061               & 4.68260                  \\
        Two hbb jets            & 4089     & 0.00761                & 0.00036                  \\
        VBF jets                & 1559     & 38.12668               & 0.00014                  \\
        VBF cut                 & 201     & 12.89288               & 0.00002                  \\
        SR                      & 0         & 0.00000                & 0.00000                  \\\hline
    \end{tabular}
\end{table}
...
A deep feed-forward neural network is employed for signal to background separation. This neural network's training uses a novel approach \textsc{neos}, which is discussed in detail in chapter \ref{sec:analysis_optimization} alongside other \ac{ml} concepts.

Inputs to the neural network include 23 features, which are the four vector components ($\pt,\eta,\phi,m$) of: the Higgs boson pair system, the individual Higgs candidates and the two \ac{vbf} jets. In addition the leading score of the GN2X tagger, the mass of the \ac{vbf} jet system $m_{j_1,j_2}$ and their pseudorapidity difference $|\Delta\eta(j_1,j_2)|$ are used. The \ac{nn} needs to be applicable across all samples, therefore only the leading GN2X score is incorporated, as the background estimation relies on a single GN2X tag.
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{tomatos_cls_5_2500_slope_50_ratios_0p8_0p5_NOSYS.SR_xbb_2_nominal_hist.pdf}
    \caption[]{\ac{nn} response for the data-driven background estimate, rescaled \ac{vbf}  \ac{sm} and \ac{ggf} signal and a \ac{vbf} signal with using a coupling $\kappa_\text{2V}=0$. The background uncertainty is calculated from error propagation of the statistical uncertainties of the unscaled background estimate and the statistical uncertainty of the weight $w_{CR}$ used in the ABCD method. It is noted that despite the appearance of almost no events in some background bins, the downscaling factor is of order $10^{-3}$ applied to events of order $10^1$.}
...
\end{figure}
The method is tested on the mc generated $\ktwov=0$ sample. Figure \ref{fig:tomatos_cls_5_nominal_reweighted_ratio} reveals a up to $\sim$\qty[]{12}{\percent} deviation for this approach. Additionally, it is noted that this method possibly suffers from a sample choice, which only explores the $\ktwov$ phase space for two $\ktwov$ couplings.
\begin{figure}
    \centering
    \includegraphics[width=.6\textwidth]{tomatos_cls_5_2500_slope_50_ratios_0p8_0p5_reweighted_nominal_ratio.pdf}
