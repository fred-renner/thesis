...
Two small radius $R=0.4$ jets are required for the \ac{vbf} signature and are referred to as \ac{vbf} jets in the following. They are also reconstructed with the anti-$k_t$ algorithm and as \acp{pfo} as described in \ref{sec:jets}. The tight \ac{wp} for the \ac{jvt} and the LooseBad \ac{wp} for the event cleaning are applied both described in \ref{sec:calibration}. Small-$R$ jets $j$ are selected for $\pt>\qty[]{20}{GeV}$ and $|\eta|<4.5$ and are required to be outside of the Higgs candidate large-$R$ jets $J$ by imposing $\Delta R(J,j) > 1.4$. \red{lets mention later that a previous analysis applied these in line with what one expects from a matrix calculation. Further cuts applied on the \ac{vbf} jet system optimized on significance are $|\Delta\eta(j,j)| > 3$ and $m_{jj} > \qty{1}{TeV}$. however if neos finds them to tight, some other cuts are probably too tight...}
...
\red{how about showing in the analysis strategy section, a fully optimized tomatos cls 5, in neos chapter the fair comparison, redo all plots then here }
...
The ABCD method is validated in the \ac{vr} within statistical uncertainties as displayed in figure \ref{fig:bkg-validation}. \red{decide for w or w vec [0,1,{2,3,4}]}
\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{tomatos_cls_5_NOSYS.VR_xbb_2_compareABCD}
    \caption[]{\red{The background estimated in the \ac{vr} from data with 1 GN2X tag, agrees with data in the \ac{vr} with 2 GN2X tags within statistical uncertainties.}}
...
\begin{figure}
    ...
    \caption[]{Comparison between a \ac{mc}-generated $\ktwov=0$ signal sample and a signal sample created with the reweighting approach by linearly combining different \ktwov hypotheses. \red{whats with the mc stat error, after rewewighting mc stat error definition becomes useless... error propagation of the stat uncertainties from the different samples }}
