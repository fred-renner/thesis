...
\begin{figure}[H]
    ...
    \caption[]{left: A quark interacting with a gluon. middle and right: triplet and quartic self coupling of gluons. Adopted from \citep{thomson2013modern}.}
...
\end{equation}
Therefore with increasing momentum transfer or viewed in a collision as particles getting closer to each other the coupling at the virtual photon vertex increases as can be seen qualitatively in figure \ref{fig:renorm_scaling}(a).
\begin{figure} 
    \centering
...
\begin{figure}
    ...
    \label{fig:qed_diagrams}
\end{figure}
Renormalization in \ac{qcd} can be derived similarly but also the quartic and triplet couplings exemplified in figure \ref{fig:qed_diagrams} need to be considered resulting in a scaling for the strong coupling 
\begin{equation}
    % \alpha_s(\mu_R^2)=\frac{12\pi}{(33-2n_f)\ln(\frac{\mu_R^2}{\Lambda^2_\text{QCD}})}.
    \alpha_S(q^2)=
    \frac{\alpha_S(\mu^2)}
    {1+B\alpha_S(\mu)
        \ln
...
\end{equation}
For 3 color charges $N_c$ and 6 fermions in the \ac{sm}, $B$ is positive and the coupling becomes weaker for shorter scales or higher momentum transfer as can be seen in figure \ref{fig:renorm_scaling}(b).
...
\end{figure}
Most importantly the running coupling of \ac{qed} does not disturb the perturbation ansatz that considers only the leading term as mentioned in section \ref{sec:qft} since developing about $\alpha\ll1$ remains small guaranteeing higher order terms in the perturbation series to vanish. This is not the case for \ac{qcd} where $\alpha_S$ at $q\approx\SI{1}{GeV}$ is of $\mathcal{O}(1)$ and perturbation theory breaks down for calculation on bound hadronic states and latter processes in hadronization. While perturbation theory is valid for $\alpha_S\approx 0.1$ which corresponds to $q\approx \SI{100}{GeV}$ in most cases higher order corrections must be considered in QCD calculations for processes that are of importance at the \ac{lhc}. 
...
\section{Electroweak Theory/Unification}
