...
\end{equation}
At this point the gauge fields $W^a_\mu$ are still massless because they would still break the gauge symmetry. To incorporate masses into the Lagrangian this electroweak symmetry can be broken down by the Higgs mechanism.

\section{Higgs mechanism}\label{sec:higgs_mechanism}

The Higgs mechanism breaks down the $SU(2)_L \otimes U(1)_Y$ into $U(1)_{EM}$ which is called \ac{ewsb}. The \ac{sm} Higgs is a single isospin doublet of complex scalar fields
\begin{equation}
    \phi(x)=
    \begin{pmatrix}
        \phi^+(x) \\
